...
interface OngoingConditionsDialogProps {
  open: boolean;
...
export function OngoingConditionsDialog({
  open,
  onOpenChange,
  onInsert,
  currentValue = "",
}: OngoingConditionsDialogProps) {
  ...
  const handleDurationChange = (index: number, duration: string) => {
    setConditions((prev) =>
      prev.map((c, i) => (i === index ? { ...c, duration } : c))
    );
  };
  ...
  const handleAddCustom = () => {
    if (newCustomName.trim()) {
      setCustomConditions((prev) => [
        ...prev,
        { name: newCustomName.trim(), duration: "" },
      ]);
      setNewCustomName("");
    }
  };
  ...
  const handleInsert = () => {
    const selectedConditions = conditions
      .filter((c) => c.checked)
      .map((c) => (c.duration ? `${c.name} (${c.duration})` : c.name));
