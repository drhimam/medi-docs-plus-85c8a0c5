...
export function MedicationsDialog({
  open,
  onOpenChange,
  onInsert,
  currentValue,
}: MedicationsDialogProps) {
  ...
  const handleMedicationToggle = (medName: string) => {
    ...
    if (existing) {
      ...
      setMedications([
        ...medications,
        { name: medName, dose: "", frequency: "", selected: true },
      ]);
    }
  };
  ...
  const updateMedication = (
    ...
    setMedications(medications.map((m) => (m.name === name ? { ...m, [field]: value } : m)));
  };
  ...
  const filteredCategories = commonMedications
    .map((cat) => ({
      ...cat,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <PresetDialogLayout
        ...
        description="Choose from common medications and add dose/frequency details"
      ...
      >
        <div className="space-y-4">
          ...
          <SelectedItemsAccordion title="Selected Medications" count={medications.length}>
            {medications.map((med) => (
              <div
                key={med.name}
                className="flex items-center gap-2 flex-wrap bg-background p-2 rounded"
              >
                ...
                <Input
                  ...
                  value={med.frequency}
                  onChange={(e) =>
                    updateMedication(med.name, "frequency", e.target.value)
                  }
                  className="w-36 h-8"
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                  {category.medications.map((med) => (
                    <div key={med} className="flex items-center space-x-2">
                      ...
                      />
                      <label htmlFor={`med-${med}`} className="text-sm cursor-pointer">
                        {med}
                      </label>
                    </div>
                  ))}
