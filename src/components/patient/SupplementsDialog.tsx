...
export function SupplementsDialog({
  open,
  onOpenChange,
  onInsert,
  currentValue,
}: SupplementsDialogProps) {
  ...
  const updateSupplementDose = (name: string, dose: string) => {
    setSupplements(supplements.map((s) => (s.name === name ? { ...s, dose } : s)));
  };
  ...
  const addCustomSupplement = () => {
    if (customSupplement.trim()) {
      setSupplements([
        ...supplements,
        { name: customSupplement.trim(), dose: customDose.trim(), selected: true },
      ]);
      setCustomSupplement("");
      setCustomDose("");
    }
  ...
  const filteredCategories = commonSupplements
    .map((cat) => ({
      ...cat,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <PresetDialogLayout
        ...
        description="Choose from common supplements and vitamins"
      ...
      >
        <div className="space-y-4">
          ...
          <SelectedItemsAccordion title="Selected Supplements" count={supplements.length}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                  {category.supplements.map((supp) => (
                    <div key={supp} className="flex items-center space-x-2">
                      ...
                      />
                      <label htmlFor={`supp-${supp}`} className="text-sm cursor-pointer">
                        {supp}
                      </label>
                    </div>
                  ))}
